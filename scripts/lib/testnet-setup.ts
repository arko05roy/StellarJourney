...
export function stellar(args: string[]): string {
  return execFileSync("stellar", args, { encoding: "utf-8", stdio: ["ignore", "pipe", "inherit"] }).trim();
}

export function stellarOk(args: string[]): boolean {
  try {
    execFileSync("stellar", args, { encoding: "utf-8", stdio: "ignore" });
    return true;
  } catch {
    return false;
  }
...
export async function fetchHorizonAccount(
  ...
  if (!response.ok) {
    throw new Error(`Horizon account lookup for ${publicKey} failed: HTTP ${String(response.status)}`);
  }
...
export async function ensureFundedIdentity(scope: string, network: string, horizonUrl: string, name: string): Promise<string> {
  ...
  if (publicKey === undefined) {
    ...
    stellar(["keys", "generate", "--fund", "--network", network, name]);
    publicKey = stellar(["keys", "address", name]);
  } else {
    const account = await fetchHorizonAccount(horizonUrl, publicKey);
    if (account === undefined) {
      log(scope, `identity "${name}" (${publicKey}) exists locally but has no funded account yet — funding via friendbot`);
      const response = await fetch(`https://friendbot.stellar.org/?addr=${publicKey}`);
      if (!response.ok) {
        throw new Error(`friendbot funding for ${name} (${publicKey}) failed: HTTP ${String(response.status)}`);
      }
    }
  }
...
export function hasTrustline(balances: HorizonBalance[], assetCode: string, issuerPublicKey: string): boolean {
  return balances.some((b) => b.asset_type !== "native" && b.asset_code === assetCode && b.asset_issuer === issuerPublicKey);
}
...
export async function ensureTrustline(
  ...
  stellar(["tx", "new", "change-trust", "--source-account", accountName, "--line", `${assetCode}:${issuerPublicKey}`, "--network", network]);
}
...
export async function ensureAssetBalance(
  ...
  const account = await fetchHorizonAccount(horizonUrl, accountPublicKey);
  const existing = account?.balances.find((b) => b.asset_type !== "native" && b.asset_code === assetCode && b.asset_issuer === issuerPublicKey);
  // Horizon reports classic-asset balances as decimal strings with 7 fractional digits.
  const existingBaseUnits = existing !== undefined ? BigInt(Math.round(Number(existing.balance) * 10_000_000)) : 0n;
