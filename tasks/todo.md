...
- [ ] `MandateStatus`, `AmountRule`, `Mandate`, `PaymentReceipt`, `RefundReceipt` (PLAN.md §10.3, §12)
- [ ] `Error` enum — all 20 codes from CLAUDE.md §8, stable numeric discriminants (frozen once assigned)
- [ ] Storage keys: `Mandate(id)`, `Payment(id)`, `UsedCharge(mandate_id, charge_id)`, `UsedRefund(refund_id)`, `RefundedTotal(payment_id)`
- [ ] Storage TTL/bump strategy — persistent for mandates+receipts, decide instance vs persistent explicitly
- [ ] `mandate_id` derivation: `hash(network_id, contract_addr, payer, merchant, asset, client_nonce)` (PLAN.md §10.2)
- [ ] Checked-arith helpers → `ArithmeticOverflow`
