  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/merchant/products/new" className={buttonVariants({ size: "sm" })} data-testid="new-product-link">
          ...
          <TableBody>
            {products.map((product) => (
              <TableRow key={product.id} data-testid={`product-row-${product.id}`}>
                ...
                <TableCell>
                  {product.amountType === "fixed" ? product.fixedAmount : `Up to ${product.maxPerCharge ?? ""}`}
                </TableCell>
                <TableCell>{formatBillingFrequency(BigInt(product.periodSeconds))}</TableCell>
                <TableCell className="text-muted-foreground">{formatAssetSymbol(product.assetAddress)}</TableCell>
                <TableCell>
