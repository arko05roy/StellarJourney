...
export default async function CheckoutLinksPage({ searchParams }: { searchParams: Promise<{ productId?: string }> }) {
  const apiKey = await requireMerchantApiKey();
  const { productId } = await searchParams;
  const [products, sessions] = await Promise.all([listProducts(apiKey), listCheckoutSessions(apiKey)]);
    ...
    <div className="flex flex-col gap-6">
      <Card className="max-w-xl">
        <CardHeader>
          ...
          <CardDescription>Share this link with a customer to start the automatic-payment authorization flow.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="flex flex-col gap-4">
        ...
        {sessions.length === 0 ? (
          <EmptyState icon={<Link2 className="size-6" />} title="No checkout links yet" description="Generate one above to see it here, along with its status." />
        ) : (
            ...
            <TableBody>
              {sessions.map((session) => (
                <TableRow key={session.id} data-testid={`checkout-session-row-${session.id}`}>
                  <TableCell className="font-medium">{productNameById.get(session.productId) ?? session.productId}</TableCell>
                  <TableCell>
