    ...
    <div className="flex flex-col gap-4">
      ...
      {payments.length === 0 ? (
        <EmptyState
          ...
          description="Confirmed on-chain charges will appear here."
        />
      ) : (
          ...
          <TableBody>
            {payments.map((payment) => (
              <TableRow key={payment.paymentId} data-testid={`payment-row-${payment.paymentId}`}>
                ...
                <TableCell>
                  <Link
                    href={`/merchant/mandates/${payment.mandateId}`}
                ...
                <TableCell className="text-right">
                  <Link
                    href={`/merchant/payments/${payment.paymentId}/refund`}
                    className={buttonVariants({ variant: "outline", size: "sm" })}
                    data-testid={`refund-link-${payment.paymentId}`}
                  >
                    Refund
                  </Link>
                </TableCell>
