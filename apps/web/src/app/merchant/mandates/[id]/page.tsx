...
import { formatMandateAmountRule, resolveAssetDecimals, toBigintMandate } from "@/lib/merchant-mandate-display";
import { formatAmount, formatAssetSymbol, formatBillingFrequency, formatDate, formatMinInterval } from "@/lib/format";
...
export default async function MandateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [products, payments] = await Promise.all([listProducts(apiKey), listPayments(apiKey, { mandateId: id })]);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <Link href="/merchant/mandates" className="text-sm text-muted-foreground hover:text-foreground hover:underline">
      ...
      <Card data-testid="mandate-detail-card">
        <CardHeader className="flex flex-row items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">Mandate {mandate.id.slice(0, 10)}…</p>
          </div>
          <MandateStatusBadge status={mandate.status} />
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          <dl className="divide-y divide-border">
            <Field label={mandate.amountRule.kind === "fixed" ? "Amount" : "Maximum charge"} value={`${formatMandateAmountRule(mandate, decimals)} ${assetSymbol}`} />
            <Field label="Maximum per billing period" value={`${formatAmount(bigintMandate.maxPerPeriod, decimals)} ${assetSymbol}`} />
            <Field label="Billing frequency" value={formatBillingFrequency(bigintMandate.periodSeconds)} />
            <Field label="Minimum interval" value={formatMinInterval(bigintMandate.minIntervalSeconds)} />
            ...
            />
            <Field label="Next eligible charge" value={nextEligible !== undefined ? formatDate(nextEligible) : "None — no future charge is possible"} />
          </dl>
          <PeriodUsageMeter usage={periodUsage} assetDecimals={decimals} assetSymbol={assetSymbol} />
        </CardContent>
      </Card>
      ...
      <div className="flex flex-col gap-4">
        ...
        {payments.length === 0 ? (
          <EmptyState icon={<Receipt className="size-6" />} title="No successful charges yet" description="Confirmed on-chain charges against this mandate will appear here." />
        ) : (
            ...
            <TableBody>
              {payments.map((payment) => (
                <TableRow key={payment.paymentId}>
                  ...
                  <TableCell className="text-right text-muted-foreground">{new Date(payment.createdAt).toLocaleString()}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
