...
const EVENT_PRODUCER_STATUS: ReadonlyArray<{ event: string; producing: boolean; note: string }> = [
  {
    ...
    producing: true,
    note: "Fired by the relayer's charge pipeline on every confirmed charge.",
  },
  {
    event: "payment.failed",
    producing: true,
    note: "Fired on a permanently failed charge attempt (not on every retry).",
  },
  {
    event: "mandate.completed",
    producing: true,
    note: "Fired when a charge brings the mandate to its maximum charge count.",
  },
  {
    event: "mandate.active",
    producing: true,
    note: "Phase 12c: the on-chain event indexer observes the contract's own mandate_created event.",
  },
  {
    event: "mandate.paused",
    producing: true,
    note: "Phase 12c: the on-chain event indexer observes the contract's own mandate_paused event.",
  },
  {
    event: "mandate.resumed",
    producing: true,
    note: "Phase 12c: the on-chain event indexer observes the contract's own mandate_resumed event.",
  },
  {
    event: "mandate.revoked",
    producing: true,
    note: "Phase 12c: the on-chain event indexer observes the contract's own mandate_revoked event.",
  },
  {
    event: "refund.succeeded",
    producing: false,
    note: "No relayer pipeline submits refund transactions on-chain yet.",
  },
];
...
export default async function WebhooksPage() {
  ...
  const [status, deliveries] = await Promise.all([
    getWebhookEndpointStatus(apiKey),
    ...
    <div className="flex flex-col gap-6">
      <Card className="max-w-xl">
        <CardHeader>
          ...
          <CardDescription>
            {status.configured
              ? `Currently sending to ${status.webhookUrl ?? ""}.`
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Which of the 8 protocol events this deployment actually produces today.
          </CardDescription>
        </CardHeader>
        <CardContent className="divide-y divide-border">
          {EVENT_PRODUCER_STATUS.map((entry) => (
            <div
              ...
              data-testid={`event-producer-${entry.event}`}
            >
              <div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {deliveries.length === 0 ? (
          <EmptyState
            ...
            description="Once events start flowing, their delivery status and attempt counts will appear here."
          />
        ) : (
            ...
            <TableBody>
              {deliveries.map((delivery) => (
                <TableRow key={delivery.id} data-testid={`webhook-delivery-row-${delivery.id}`}>
                  ...
                  <TableCell>
                    <Badge variant={DELIVERY_STATUS_VARIANT[delivery.status] ?? "outline"}>
