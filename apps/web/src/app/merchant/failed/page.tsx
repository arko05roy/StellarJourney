    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Most of these are the protocol correctly blocking a charge that did not match the
          mandate's own terms — not a bug, and no money moved. A smaller number are temporary
          network/submission issues, marked separately below.
        </p>
      </div>
      {charges.length === 0 ? (
        <EmptyState
          ...
          description="Blocked or failed charge attempts will appear here with the exact reason."
        />
      ) : (
        <Card data-testid="failed-collections-list">
          <CardContent className="divide-y divide-border">
            {charges.map((charge) => {
              ...
              const isInfra =
                charge.failureCode !== undefined && INFRA_TRANSIENT_CODES.has(charge.failureCode);
              return (
                <div
                  key={charge.id}
                  className="flex items-start justify-between gap-4 py-3"
                  data-testid={`failed-charge-row-${charge.id}`}
                >
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {charge.amount} ·{" "}
                      <Link
                        href={`/merchant/mandates/${charge.mandateId}`}
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1">
                    <Badge
                      variant={isInfra ? "secondary" : "outline"}
                      title={
                        isInfra
                          ? "Temporary network/submission issue, not a policy rejection"
                          : "The mandate's own rules correctly blocked this charge"
                      }
                    >
