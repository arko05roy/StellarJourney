...
function toMandateResponse(mandate: Mandate) {
  return {
    ...
    lastChargedAt:
      mandate.lastChargedAt !== undefined
        ? new Date(Number(mandate.lastChargedAt) * 1000).toISOString()
        : undefined,
...
const mandatesRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get(
    "/mandates",
    { preHandler: createAuthPreHandler(app.prisma, app.hashSecret, ["mandates:read"]) },
    async (request, reply) => {
