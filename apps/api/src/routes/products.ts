...
export function toProductResponse(product: Product) {
  ...
  return {
    ...
    fixedAmount: product.fixedAmount ? baseUnitsToDecimalString(BigInt(product.fixedAmount), decimals) : undefined,
    maxPerCharge: product.maxPerCharge ? baseUnitsToDecimalString(BigInt(product.maxPerCharge), decimals) : undefined,
...
const productsRoutes: FastifyPluginAsync = async (app) => {
  app.post("/products", { preHandler: createAuthPreHandler(app.prisma, app.hashSecret) }, async (request, reply) => {
    const { merchant } = requireMerchantContext(request);
    const input = CreateProductSchema.parse(request.body);

    const maxPerPeriodBaseUnits = toPositiveBaseUnitsOrBadRequest("maxPerPeriod", input.maxPerPeriod, input.assetDecimals);
    const fixedAmountBaseUnits =
      input.amountType === "fixed" ? toPositiveBaseUnitsOrBadRequest("fixedAmount", input.fixedAmount, input.assetDecimals) : undefined;
    const maxPerChargeBaseUnits =
      input.amountType === "variable" ? toPositiveBaseUnitsOrBadRequest("maxPerCharge", input.maxPerCharge, input.assetDecimals) : undefined;
  ...
  app.get("/products/:id", { preHandler: createAuthPreHandler(app.prisma, app.hashSecret) }, async (request, reply) => {
    ...
    const product = await app.prisma.product.findFirst({ where: { id, merchantId: merchant.id } });
    if (!product) {
